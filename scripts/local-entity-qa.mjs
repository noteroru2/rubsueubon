...
for (const district of districts) {
  ...
  assert(!mainBusiness?.geo && !mainBusiness?.hasMap, `${district.slug}: unverified main geo/map remains in LocalBusiness`);
  assert(!mainBusiness?.address?.streetAddress, `${district.slug}: unverified streetAddress remains in LocalBusiness`);
  ...
  assert(!schemas.some((schema) => containsValue(schema, 'GeoCoordinates')), `${district.slug}: fake district GeoCoordinates remains`);
...
assert(sitemapCount === 211, `Sitemap regression: expected 211 URLs, found ${sitemapCount}`);
